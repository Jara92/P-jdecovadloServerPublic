...
public class
    ReturnProtocolAuthorizationHandler : BaseCrudAuthorizationHandler<OperationAuthorizationRequirement, ReturnProtocol>
{
    public ReturnProtocolAuthorizationHandler(IAuthenticateService authenticateService) : base(authenticateService)
    {
    }

    protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context,
        OperationAuthorizationRequirement requirement, ReturnProtocol protocol)
    {
        ...
        switch (requirement.Name)
        {
            ...
            case nameof(Operations.Update):
                // Only the owner can update the protocol but the loan return must be denied
                if (protocol.Loan.Item.Owner.Id == userId)
                    context.Succeed(requirement);
