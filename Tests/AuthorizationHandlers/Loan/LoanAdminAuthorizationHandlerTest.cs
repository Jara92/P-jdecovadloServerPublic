...
public class LoanAdminAuthorizationHandlerTest
{
    LoanAdminAuthorizationHandler _authorizationHandler;
    ...
    public void Setup()
    {
        ...
        _authorizationHandler = new LoanAdminAuthorizationHandler(_authenticateService.Object);
    ...
    public async Task HandleAsync_UserHasAdminRole_ReturnsTrue()
    {
        ...
        {
            var context =
                new AuthorizationHandlerContext(new List<IAuthorizationRequirement> { requirement }, _admin, _loan);
            await _authorizationHandler.HandleAsync(context);
