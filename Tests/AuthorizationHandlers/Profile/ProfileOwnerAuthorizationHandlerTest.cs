...
public class ProfileOwnerAuthorizationHandlerTest
{
    ProfileOwnerAuthorizationHandler _authorizationHandler;
    ...
    public void Setup()
    {
        ...
        _authorizationHandler = new ProfileOwnerAuthorizationHandler(_authenticateService.Object);
    ...
    [Test]
    public async Task Read_UserIsProfileOwner_ReturnsTrue()
    {
        // Mock user
    ...
    [Test]
    public async Task Create_UserIsProfileOwner_ReturnsTrue()
    {
        // Mock user
        _authenticateService.Setup(x => x.TryGetCurrentUserId()).Returns(_owner.FindFirstValue(ClaimTypes.Sid));

        var context =
            new AuthorizationHandlerContext(new List<IAuthorizationRequirement> { ProfileOperations.Create }, _owner,
                _userProfile);
        await _authorizationHandler.HandleAsync(context);

        // Owner cannot create a profile
        Assert.That(context.HasSucceeded, Is.False);
    }

    [Test]
    public async Task Update_UserIsProfileOwner_ReturnsTrue()
    {
        // Mock user
        _authenticateService.Setup(x => x.TryGetCurrentUserId()).Returns(_owner.FindFirstValue(ClaimTypes.Sid));

        var context =
            new AuthorizationHandlerContext(new List<IAuthorizationRequirement> { ProfileOperations.Update }, _owner,
                _userProfile);
        await _authorizationHandler.HandleAsync(context);

        // Owner can update his profile
        Assert.That(context.HasSucceeded, Is.True);
    }

    [Test]
    public async Task Delete_UserIsProfileOwner_ReturnsTrue()
    {
        // Mock user
        _authenticateService.Setup(x => x.TryGetCurrentUserId()).Returns(_owner.FindFirstValue(ClaimTypes.Sid));

        var context =
            new AuthorizationHandlerContext(new List<IAuthorizationRequirement> { ProfileOperations.Delete }, _owner,
                _userProfile);
        await _authorizationHandler.HandleAsync(context);

        // Owner cannot delete his profile
        Assert.That(context.HasSucceeded, Is.False);
    }
}